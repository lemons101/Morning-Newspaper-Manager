...
plan_map = {str(x.get('item_id') or ''): x for x in plan_items if isinstance(x, dict)}
merged = []
subagent_hits = 0
fallback_hits = 0
for item in triage_items:
    ...
    if str(item.get('channel') or '') == 'mail_alert':
        ...
        merged.append(row)
        continue
    plan_item = plan_map.get(str(item.get('item_id') or ''))
    result = plan_item.get('result') if isinstance(plan_item, dict) else None
...
payload = {
    'selection_method': 'llm_candidate_subagent_merge_v2',
    'count': len(merged),
...
print(f'wrote {len(merged)} items to {OUT} | subagent_hits={subagent_hits} fallback_hits={fallback_hits}')
